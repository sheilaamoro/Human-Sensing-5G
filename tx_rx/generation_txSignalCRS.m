...
% Date: June, 2025
% File: generation_txSignalCRS.m
%
% Description:
% This function generates the baseband OFDM signal to be transmitted when the 
% sensing sequence is CRS. It maps the ZC sequence onto an OFDM resource
...
%
% function [txSignal, dataGrid, nullIndices] = generation_txSignalCRS(Nfft, Ncp, ofdmSampleRate, N_crs, NofdmSyms)
%
% Input parameters:
%     Nfft: Total number of subcarriers (FFT size)
%     Ncp: Lenght of the cyclic prefix
%     ofdmSampleRate: Sampling rate (defines bandwidth)
%     N_crs: Lenght of the CRS sequence
...
% ---- Generate CRS Sequence ---- 
s_crs = genCRS(N_crs, 0, 0, 0, 'normal', false);

% ---- OFDM Data Grid ----
% The sequence is mapped over Nfft subcarriers, with N_crs active subcarriers. 
leftGuard = floor((Nfft - (N_crs + 1)) / 2);        
activeIndices = (leftGuard + 1):(leftGuard + N_crs + 1);  
DC_index = Nfft/2 + 1; % DC subcarrier index             
...
dataSymbol_full = [s_crs(1:dcDataIdx-1); 0; s_crs(dcDataIdx:end)]; 
dataSymbol = dataSymbol_full;
dataSymbol(dcDataIdx) = []; % Remove DC for ofdmod

% Repeat the same data symbol across all OFDM symbols
dataGrid = repmat(dataSymbol, 1, NofdmSyms); 

% Define null subcarriers
nullIndices = [1:leftGuard, DC_index, (leftGuard+N_crs+2):Nfft].';
...
if hasNulls
    ...
    customColormap = [0.95 0.55 0.55;  % Red for DC
                      0.30 0.75 0.60]; % Green for CRS Sequence
